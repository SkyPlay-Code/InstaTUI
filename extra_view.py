...
class ExtraView(Vertical):
    def compose(self):
        with Vertical(id="extra-container"):
            yield Label("🧰 EXTRA TOOLS: Data Archiver", id="extra-header")
            
            with Horizontal(classes="action-row"):
                yield Button("1. Fetch Available Threads", id="load-threads-btn", variant="primary")
            
            yield OptionList(id="dl-thread-list")
            
            with Horizontal(classes="action-row"):
                yield Input(placeholder="Messages to dl (leave blank for ALL)", id="dl-limit-input")
                yield Button("2. Start Download", id="start-dl-btn", variant="success", disabled=True)
